...
| V06 | Preguntas de Costa Rica | hecho | 1 | (este commit) | ver V07 | Playwright sobre / : 15 preguntas visibles = 15 en JSON-LD FAQPage, preguntas y respuestas idénticas 1:1 (qMatch/aMatch true); orden precio → incluye → pago → factura → contrato → plazos → cambios → hardware → lo demás; '1 año de soporte' = 0; cifras de primer mes generadas con firstPaymentFor (₡39 800 / ₡149 900 / ₡174 900) = planes; contrato y cancelación = términos (15 días); qa:landing OK |
